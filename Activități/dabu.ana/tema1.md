...
comandă complexă:* grep bash /etc/passwd | awk -F: '{print $7}' | sort -u Ar trebui să vedeți un răspuns care arată ca /bin/bash sau /usr/local/bin/bash.
Pentru a instala bash ca shell de conectare, tastați chsh nume-bash, unde nume-bash este răspunsul pe care l-ați primit la comanda whereis (sau orice altceva care a funcționat). De exemplu: % chsh /usr/local/bin/bash
...
De exemplu, linia de comandă lp myfile constă din comanda lp (print a file)și argumentul unic myfile. lp tratează myfile ca nume de fișier de imprimat. Argumentelesunt adesea nume de fișiere, dar nu neapărat: în linia de comandă mail cam,programul de mail tratează cam ca nume de utilizator către care va fi trimis un mesaj.
...
Comanda lp -h myfile conține opțiunea -h, care îi spune lui lp să nu imprime „pagina banner” înainte de a imprima fișierul.
